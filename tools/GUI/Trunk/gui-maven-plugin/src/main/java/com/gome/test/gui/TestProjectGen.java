...
public class TestProjectGen {
    ...
    class CaseFileAndList {
        ...
        CaseFileAndList(String prefix, List<String> caseList,TreeMap<String,String> caseIdCategoryMap) {
            this.prefix = prefix;
            this.caseList = caseList;
            this.caseIdCategoryMap=caseIdCategoryMap;
        }

        String prefix;
        List<String> caseList;
        TreeMap<String,String>  caseIdCategoryMap;
                ...
                if (file.isDirectory()) {
                    ...
                    try {
                        ...
                        for (Case caze : cases) {
                            ...
                            this.testCaseList.put(caze.getId(), new CaseFileAndList(newMethodPrefix, caseList, caze.getCaseCategory() ));
                        }
    ...
    private void updateCaseCategory(File caseCategory) throws Exception {
        ...
        {
           if(testCaseList.get(key).caseIdCategoryMap!=null && testCaseList.get(key).caseIdCategoryMap.size()>0)
           {
               for(String k : testCaseList.get(key).caseIdCategoryMap.keySet())
               {
                   categorySet.add(k);
               }
           }
        ...
        for (String key : testCaseList.keySet()) {
            ...
            for (int t = 0; t < content.size(); t++) {
                ...
                if (con.get(0).equals(caseId)) {
                    ...
                    for (int i = categoryBeginColIndex; i < headCon.size(); i++) {
                        String headkey= headCon.get(i);
                        String value="";
                        if(testCaseList.get(key).caseIdCategoryMap!=null && testCaseList.get(key).caseIdCategoryMap.get(headkey) != null)
                            value = testCaseList.get(key).caseIdCategoryMap.get(headkey);
            ...
            if (!boo) {
                ...
                for (int i = categoryBeginColIndex; i < headCon.size(); i++) {
                    ...
                    if(testCaseList.get(key).caseIdCategoryMap!=null && testCaseList.get(key).caseIdCategoryMap.get(headkey) != null) {
                        value = testCaseList.get(key).caseIdCategoryMap.get(headkey);
                        emt.set(categoryColIndexMap.get(headkey), value);
                    }
                }
