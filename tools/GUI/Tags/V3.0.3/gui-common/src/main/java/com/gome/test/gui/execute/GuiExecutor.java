...
//        initScreenCaptureDir();ide使用 不用截图
    ...
    public void run(String caseId, boolean closeBrowseAfterCompleted) throws Throwable {
        for (Case c : cases) {
            ...
            String configFilePath = String.format("%s%s%s", getResourcesPath(this.filePath), File.separator, Settings.SELENIUM_PROPERTIES);
    ...
    private Object getMethodValue(String param, String packageName, Class<?>[] paramTypes) throws Exception {
        String dataFilePath = String.format("%s%s%s%sdata.xlsx", getResourcesPath(filePath), File.separator,
                getRelativePath(new File(filePath)).replace(
