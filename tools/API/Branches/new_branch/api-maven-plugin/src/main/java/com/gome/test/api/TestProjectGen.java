...
public class TestProjectGen {
   ...
   * */
            ...
            for (File file : testCaseDir.listFiles()) {
                ...
                if (file.isDirectory()) {
                    ...
                    try {
                        ...
                        cases.testNGCount=this.testNGCount;//设置每个Test里面最多的case数量
                        //验证Case名称或者Id是否重复
                        String caseNameOrcaseId = getExistsCaseIdorName(String.format("%s_%s_", methodPrefix, filename), cases.getTestCases());

                        if (caseNameOrcaseId != null) {
                            //记录异常日志
                            if (null != logger) {
                                logger.info(String.format("%s\\%s, The %s  already exists ", testCaseDir.getName(), file.getName(), caseNameOrcaseId));
                            }
                            throw new Exception(String.format("文件%s\\%s的用例:%s，重复了，无法继续执行，请修改！ ", testCaseDir.getName(), file.getName(), caseNameOrcaseId));
                        }
    ...
    public void genTestProject(String groupId, String artifactId,
                               String version, File testProjectDir, File testCaseDir, String timeout,
         ...
         */
        TestProject project = new TestProject("com.gome.test.api","api-helper-parent","RELEASE",groupId, artifactId, version);
        ...
        try {
            for (AtomicCase atomicCase : cases.getTestCases()) {
                if ((indexCategory = Arrays.asList(atomicCase.getParamKeys()).indexOf("caseCategory")) != -1) {
                    caseIdCategoryMap.put(atomicCase.getId(), JsonUtils.readValue(atomicCase.getParamValues()[indexCategory].toString(), TreeMap.class));
                }
            }
        } catch (Exception e) {
