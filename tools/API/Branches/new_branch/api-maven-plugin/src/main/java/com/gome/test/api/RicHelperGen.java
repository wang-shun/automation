...
//            Logger.error("%s 不存在", this.dubboConfigFile.getAbsolutePath());
    ...
    private void generateDir(String jarName, Class<?> c) throws ClassNotFoundException, IOException {
        String packageName;
        packageName = "com.gome.test.api." + Utils.getClassLastName(c).toLowerCase();
...
//                if (dubboConfigFileContent.indexOf(String.format("id=\"%s\"", Utils.changeTheFirstCharToLower(className))) == -1) {
//                    String dubbo = String.format(Constant.DUBBO_SPRINT_CONFIG, Utils.changeTheFirstCharToLower(className), classFullName);
