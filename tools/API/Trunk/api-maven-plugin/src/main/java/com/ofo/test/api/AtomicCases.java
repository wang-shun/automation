...
public class AtomicCases {
    ...
    public int testNGCount;
    private List<List<AtomicCase>> testCasesList;
    ...
    public void setTestCasesList(){

        if(this.testNGCount==0)
            this.testNGCount=Constant.TESTNGCOUNT;

        List<List<AtomicCase>> lists=new ArrayList<List<AtomicCase>>();
        for(int i=0;i< testCases.size();i++)
        {
            if(i==0 || i >= (lists.size() * this.testNGCount))
            {
                lists.add(new ArrayList<AtomicCase>());
            }
            lists.get(lists.size()-1).add(testCases.get(i));

        }
        this.testCasesList=lists;
        ...
        for (int i = 0; i < sheetNum; ++i) {
            ...
            if (!sheetName.startsWith(Constant.P_SUITES_)) {
                if ((!sheetName.startsWith(Constant.TEST_SUITES))) {
                    if ((!sheetName.startsWith(Constant.DUBBO_SUITES))) {
                        continue;
                    }
                }
            }
            // process sheet depending on different sheet name
            if ((sheetName.startsWith(Constant.TEST_SUITES) || sheetName.startsWith(Constant.DUBBO_SUITES)) && !(sheetName.startsWith(Constant.TEST_SUITES_FOR_DEV))) {
    ...
    public List<List<AtomicCase>>  getTestCasesList() {
