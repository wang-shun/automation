...
public class ParamizedSheet implements ExcelSheetDataSource {
    ...
    public ClientEnum parse(Sheet dataSheet, Workbook workbook, List<AtomicCase> testCases, GlobalSettings settings) {
        ...
        String templateName = Constant.P_SUITES_TPL_ + sheetNameArray[2];
        ...
        int caseParameterIndex = pos.get(varaiableKey.indexOf(Constant.CASE_PARAM));    //p-case
        String[] removeHeadersArray = {Constant.CASE_NO, Constant.CASE_NAME, Constant.OWNER_FIRST_UPPER, Constant.CASE_PARAM};
        ...
        while (rowIter.hasNext()) {
            ...
            int verifyClassIndex = paramKeyArray.indexOf(Constant.VERIFY_CLASS);      //template case
            int verifyStepsIndex = paramKeyArray.indexOf(Constant.VERIFY_STEPS);      //template case
            int caseVariablesIndex = paramKeyArray.indexOf(Constant.CASE_VARIABLES);  //template case
