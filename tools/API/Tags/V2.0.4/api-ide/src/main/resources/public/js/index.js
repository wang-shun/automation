...
$('#nav-update-workbench').on('click', function () {
    $('#modal-loading').modal('show');
    $.ajax({
        type: 'POST',
        ...
        success: function (data) {
            if (!data['isError']) {
                ...
                $('#modal-loading').modal('hide');
                $('#status').text('更新工作区失败：' + data['message']);
                $('#modal-status').modal('show');
            }
        }
    });
...
$('#nav-commit-workbench').on('click', function () {
    $('#modal-loading').modal('show');
    $.ajax({
        type: 'POST',
        url: 'workbench/commit',
        success: function (response) {
            if (!response['isError']) {
                reloadTree();
                $('#modal-loading').modal('hide');
                ...
                $('#modal-loading').modal('hide');
                $('#status').text('提交工作区失败：' + response['message']);
                $('#modal-status').modal('show');
            }
        }
    });
...
$('#nav-shutdown').on('click', function () {
    ...
    $('body').append("WebIDE服务已经成功关闭，谢谢使用！");
});
