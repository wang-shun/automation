...
public class TestProjectGen {
    ...
    public Map<String, List<String>> testCaseClassList;
    int suiteIndex = 0;
...
//                    continue;
                ...
                if (file.isDirectory()) {
                    if (file.getParent().endsWith("TestCase")) {
                        //加文件夹
                        testCaseClassList.put(filename.toLowerCase(), new ArrayList<String>());
                    }
                    ...
                    for (String key : testCaseClassList.keySet()) {
                        if (file.getParentFile().getName().toLowerCase().equals(key)) {
                            testCaseClassList.get(key).add(file.getName().replace(".xlsx", ""));
                        }
                    }
    ...
    public void genTestProject(String groupId, String artifactId,
                               ...
                               Log logger, File helpJavaApiDirectory) throws Exception {
        this.logger = logger;
        ...
        this.testCaseClassList = new HashMap<String, List<String>>();
        ...
        //根据testcase删除没有case的文件
        logger.info(String.format("开始过滤生成文件"));
        updateJavaPath(helpJavaApiDirectory);

    }

    private void updateJavaPath(File helpJavaDir) {
        for (File file : helpJavaDir.listFiles()) {
            if (checkDeleteDirectoryName(file)) {//不包含忽略文件
                //检查文件夹匹配
                if (!checkDirectory(file.getName())) {
                    deleteFile(file);
                } else {//检查case匹配
                    String dirName = file.getName();
                    for (File childFile : file.listFiles()) {
                        if (checkDeleteFileName(childFile)) {
                            if (!checkfile(dirName, childFile.getName()))
     ...
     * @return
     */
    private boolean checkDeleteDirectoryName(File file)
    {
        return  !file.getName().equals(".svn") && !file.getName().equals("environment") && !file.getName().equals("verify");
    }
     ...
     * @return
     */
    private boolean checkDeleteFileName(File file)
    {
    ...
    private void deleteFile(File file) {
        ...
        if (!file.isDirectory())
        {
            ...
            if(file.delete())
            {
                logger.info(String.format("删除%s文件",fileName));
            }
        }
        else {
            for (File childFile : file.listFiles()) {
     ...
     * @return
     */
    private boolean checkDirectory(String dirName) {
        boolean reslut = false;
        dirName = dirName.toLowerCase();
        for (String name : testCaseClassList.keySet()) {
            if (name.toLowerCase().equals(dirName)) {
                reslut = true;
                break;
            }
        }
    ...
    private boolean checkfile(String dirName, String fileName) {
        ...
        for (String name : testCaseClassList.get(dirName)) {
            if (fileName.toLowerCase().indexOf(dirName + name.toLowerCase()) == 0) {
                result = true;
                break;
            }
        }
