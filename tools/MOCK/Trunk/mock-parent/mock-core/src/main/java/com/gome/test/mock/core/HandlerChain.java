 ...
 * @param <I>
 */
public class HandlerChain<O, I> {//双向循环链表封装类
	...
	public HandlerChain(Channel<O, I> channel) {
	    ...
	    head.next = tail;//头部指针指向尾部
	    tail.prev = head;//尾部指针指向头部
	}
	...
	public HandlerChain<O, I> addLast(String name, Handler<O, I> handler) {
	    ...
	    SimpleHandlerContext<O, I> ctx = new SimpleHandlerContext<O, I>(this, channel, name, handler);//创建
	    ctx.prev = tail.prev;
	    tail.prev = ctx;
     ...
     * @param <I>
     */
    static final class HeadContext<O, I> extends SimpleHandlerContext<O, I> implements Handler<O, I> {//类适配器模式
