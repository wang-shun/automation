        ...
        while (readOffset < firstTryBytes.length - 1) {
            ...
            if (headerToken.hasMoreTokens()) {
                ...
                if (headerToken.hasMoreTokens()) {
                    ...
                    if (ConstDefine.HEAD_HOST.equals(key)) {
                        if (headerToken.hasMoreTokens()) {
                            String port = headerToken.nextToken().trim();
                            shm.addHeader(ConstDefine.HEAD_PORT, port);
                        } else {
                            shm.addHeader(ConstDefine.HEAD_PORT, DEFAULT_PORT);
                        }
                    } else {
